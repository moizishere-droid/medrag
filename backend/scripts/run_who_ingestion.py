"""
Run unified WHO guideline extraction (text + tables + images) for all topics
with a real, dedicated WHO guideline.
...
Several topics intentionally share the same source document (e.g. asthma/
copd via the PEN package; coronary artery disease/heart failure/stroke/
hyperlipidemia via the CVD risk guideline; depression/anxiety disorder/
epilepsy via mhGAP) - each unique document is downloaded and extracted only
once, then its results are re-saved under every topic name that shares it.
"""
...
from medrag.ingestion.storage import save_guideline, save_who_tables, save_who_images
...
def main():
    ...
    for topic, (url, title) in TOPIC_DOCS.items():
        if url in processed_urls:
            ...
            logger.info(f"{topic}: reused cached extraction from shared document")
        ...
        try:
            ...
            save_guideline(guideline, output_dir=GUIDELINE_DIR)
            save_who_tables(tables, topic=topic, output_dir=TABLE_DIR)
            saved_images = save_who_images(images, topic=topic, output_dir=IMAGE_DIR)
